...
# 1. Client gets an output by calling createOutput with the name of the Roku Channel
def createOutput(
    providerName, baseUrl="conspyre.tv", baseUrlB="news.conspyre.tv", cdnBaseUrl="https://b-low.b-cdn.net/", imageSrcUrl="", imageCdnUrl="https://b-low.b-cdn.net"
):
...
def curlJsonDict(output, name, order, url, append, addToRecent):
    ...
    mergeOutput(
        values, output, "movies", append, addToRecent
    )  # hardcode schema for now
...
def writeOutput(output, filename):
    ...
    if makeRecent == True:
        ...
        for key in reversed(keys):
            output["playlists"][reserveIndex]["itemIds"].append(
                output["recentVideos"][key]
            )
    ...
    if moviesCount > maxVideos:
        print("*****************************************************************")
        print(
            f"*** WARNING: {filename} has {moviesCount} movies, which is >{maxVideos} 🤦‍♂️"
        )
        print("*****************************************************************")
    else:
    ...
    if categoriesCount > maxCategories:
        print("*****************************************************************")
        print(
            f"*** WARNING: this channel has {categoriesCount} categories, which is >{maxCategories} 🤦‍♂️"
        )
        print(f"*** {filename}")
        print("*****************************************************************")
    else:
...
def mergeOutput(dict, output, schema, append, addToRecent):
    ...
    for m in dict["movies"]:  # iterate incoming movies
        m["thumbnail"] = m["thumbnail"].replace(
            "_roku", ""
        )  # roku jpeg doesn't work some % of the time, but, we always seem to have a thumbnail - let's just use it
        if len(imageBase) > 0:
        ...
        if not m["id"] in output["ids"]:  # skip if we've already processed this ID
            if len(m["shortDescription"]) == 0:  # fix empty description
                m["shortDescription"] = (
                    output["providerName"] + ": " + dict["categories"][0]["name"]
                )
