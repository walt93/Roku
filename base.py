...
def download_video(url, directory):
    try:
        ...
        subprocess.run(['yt-dlp', '-S', 'res,ext:mp4:m4a', '--recode', 'mp4', '-o', output_template, url], check=True)
        video_info = subprocess.check_output(['yt-dlp', '--get-filename', '-o', output_template, url]).decode().strip()
...
#
def upload_video(file_path, title, video_password, description, categories_id, username, password, created):
    url = f'https://news.conspyre.tv/plugin/MobileManager/upload.php?user={username}&pass={password}'
    file_key = 'upl'

    with open(file_path, 'rb') as f:
        files = {file_key: f}
        data = {'title': title, 'video_password': video_password, 'description': description, 'categories_id': categories_id}
...
def scrape_rumble_video_page(video_url):
    ...
    upload_date = video_data["uploadDate"]
